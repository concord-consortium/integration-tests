// Admin page elements
const adminPageElements = {
  USERS_LINK : 'div.admin-links-cols__column:nth-child(2) a[href$=\'/users\']',
  AUTHORING_LINK : 'a[href=\'/authoring\']',
  NOTICES_LINK : 'a[href$=\'/admin/site_notices\']',
  TAGS_LINK: 'a[href$=\'/admin/tags\']',
  SCHOOLS_LINK : 'a[href$=\'/portal/schools\']',
  MATERIALS_COLLECTION_LINK : 'a[href$=\'/materials_collections\']',
  PROJECTS_LINK : 'div.admin-links-cols__column:nth-child(2) a[href$=\'/admin/projects\']',
  LNK_AUTHORING: 'a[href$=\'/authoring\']',
  LNK_ADMIN: '#clazzes_nav ul li a[href=\"/admin\"]',
  LNK_ADMIN_SETTINGS: 'li a[href=\"/admin/settings\"]',
};

export default adminPageElements;
