...
import {automatedtestLaraSequence1Data} from "../../support/testdata/testdata_automatedtestsequence1_lara";
...
const ASSIGNMENT_NAME = 'Cypress_AutomatedTestSequence1_LARA';
...
context("Verify Student Activity Work Flow", () => {
    ...
    it("Run the assignment with students added to the class", () => {
        let totalActivitiesInSequence = automatedtestLaraSequence1Data.totalActivities;
        let studentIndex = 0;
        STUDENTS.forEach(eachStudent => {
            let studentDetails = {
                username: C[eachStudent + '_USERNAME'],
            ...
            for(let activityIndex = 1 ; activityIndex <= totalActivitiesInSequence ; activityIndex++){
                let currentActivity = automatedtestLaraSequence1Data[activityIndex];
                ...
                cy.wait(2000);
                cy.get(laraSequenceElements.LBL_ESTIMATED_TIME_IN_MINUTES).should('have.text', automatedtestLaraSequence1Data.timeInMinutes);
                LaraSequenceHelper.goToActivity(activityIndex);
    ...
    it("Verify teacher can verify reports and provide feedback", () => {
        ...
        let totalActivities = automatedtestLaraSequence1Data.totalActivities;
        for(let activityIndex = 1; activityIndex <= totalActivities; activityIndex++){
            let currentActivity = automatedtestLaraSequence1Data[activityIndex];
