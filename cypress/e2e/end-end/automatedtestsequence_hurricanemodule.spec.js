...
import {automatedtestSequenceHurricaneModuleData} from "../../support/testdata/testdata_automatedtestsequence_hurricanemodule";
...
const ASSIGNMENT_NAME = 'Cypress_Automated_Hurricane_Module';
...
context("Verify Student Activity Work Flow", () => {
    ...
    it("Run the assignment with students added to the class", () => {
        let totalActivitiesInSequence = automatedtestSequenceHurricaneModuleData.totalActivities;
        let studentIndex = 0;
        STUDENTS.forEach(eachStudent => {
            let studentDetails = {
                username: C[eachStudent + '_USERNAME'],
            ...
            for(let activityIndex = 1 ; activityIndex <= totalActivitiesInSequence - 4; activityIndex++){
                ...
                let currentActivity = automatedtestSequenceHurricaneModuleData[activityIndex];
                let activityName = currentActivity.name;
                cy.log("Lara Sequence Helper");
                cy.wait(15000);
                ...
                // cy.wait(2000);
                cy.get(laraSequenceElements.LBL_ESTIMATED_TIME_IN_MINUTES).should('have.text', automatedtestSequenceHurricaneModuleData.timeInMinutes);
    ...
    it("Run the Hurricane sim as students", () => {
        let totalActivitiesInSequence = automatedtestSequenceHurricaneModuleData.totalActivities;
        let studentIndex = 0;
        STUDENTS.forEach(eachStudent => {
            let studentDetails = {
